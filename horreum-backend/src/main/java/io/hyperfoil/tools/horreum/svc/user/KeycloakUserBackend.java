...
public class KeycloakUserBackend implements UserBackEnd {
    ...
    private static final String[] ROLE_TYPES = new String[]{
            "team", Roles.VIEWER, Roles.TESTER, Roles.UPLOADER, Roles.MANAGER
    };
    ...
    public List<UserService.UserData> searchUsers(String query) {
        try {
            ...
            return keycloak.realm(realm)
                    ...
                    .stream()
                    .filter(rep -> !machineIds.contains(rep.getId()))
    ...
    public void createUser(UserService.NewUser user) {
        ...
        try (
            ...
            if (response.getStatusInfo()
                ...
                if (!keycloak.realm(realm)
                        .users()
                        .search(rep.getUsername(), true)
                        .isEmpty()) {
                    throw ServiceException.badRequest("User exists with same username");
                } else if (!keycloak.realm(realm)
                        .users()
                        .searchByEmail(rep.getEmail(), true)
                        .isEmpty()) {
                            throw ServiceException.badRequest("User exists with same email");
                        } else {
                            throw ServiceException.badRequest(
                                    "Failed to create new user: " + response.getStatusInfo()
                                            .getReasonPhrase()
                            );
                        }
            }
        ...
        try {  // assign the provided roles to the realm
            ...
            String userId = matchingUsers.get(0)
                    .getId();
            ...
            if (user.team != null) {
                ...
                if (user.roles.contains(Roles.MACHINE)) {
                    ...
                    usersResource.get(userId)
                            .roles()
                            .realmLevel()
                            .add(List.of(ensureRole(Roles.MACHINE)));
            ...
            ClientsResource clientsResource = keycloak.realm(realm)
                    .clients();
            ClientRepresentation account = clientsResource.query("account")
                    .stream()
                    .filter(c -> "account".equals(c.getName()))
                    .findFirst()
                    .orElse(null);
            if (account != null) {
                RoleRepresentation viewProfile = clientsResource.get(account.getId())
                        .roles()
                        .get("view-profile")
                        .toRepresentation();
                if (viewProfile != null) {
                    usersResource.get(userId)
                            .roles()
                            .clientLevel(account.getClientId())
                            .add(List.of(viewProfile));
                }
            }
        } catch (ServiceException se) {
            throw se; // thrown above, re-throw
    ...
    public void removeUser(String username) {
        try (
                Response response = keycloak.realm(realm)
                        .users()
                        .delete(findMatchingUserId(username))
        ) {
            if (response.getStatusInfo()
                    .getFamily() != Response.Status.Family.SUCCESSFUL) {
    ...
    public List<String> getTeams() { // get the "team roles" in the realm
        try {
            return keycloak.realm(realm)
                    ...
                    .filter(KeycloakUserBackend::isTeam)
                    .toList();
        } catch (Throwable t) {
    ...
    private String findMatchingUserId(String username) { // find the clientID of a single user
        List<UserRepresentation> matchingUsers = keycloak.realm(realm)
                .users()
        ...
        if (matchingUsers == null || matchingUsers.isEmpty()) {
            ...
            LOG.warnv(
                    "Multiple matches for exact search for username {0}: {1}",
                    username,
        ...
        for (String role : ROLE_TYPES) {
            try {
                ...
                keycloak.realm(realm)
                        ...
                        .getUserMembers(0, Integer.MAX_VALUE)
                        .forEach(
                                user -> userMap.computeIfAbsent(user.getUsername(), u -> new ArrayList<>())
                                        .add(role)
                        );
    ...
    public void updateTeamMembers(String team, Map<String, List<String>> roles) { // update the team membership. the roles provided here are "UI roles"
        ...
        for (Map.Entry<String, List<String>> entry : roles.entrySet()) {
            ...
            try { // fetch the current roles for the user
                ...
                rolesMappingResource = keycloak.realm(realm)
                        .users()
                        .get(userId)
                        .roles();
                existingRoles = rolesMappingResource.getAll()
                        .getRealmMappings()
                        .stream()
                        .map(RoleRepresentation::getName)
                        .toList();
                ...
                throw ServiceException.serverError(
                        format("Failed to retrieve current roles of user {0} from Keycloak", entry.getKey())
                );
            }
            ...
            try { // add new roles that are not in the list of current roles and then remove the existing roles that are not on the new roles
                List<RoleRepresentation> rolesToAdd = entry.getValue()
                        .stream()
                        .filter(uiRole -> !existingRoles.contains(prefix + uiRole))
                        .map(uiRole -> ensureRole(prefix + uiRole))
                        .toList();
                if (!rolesToAdd.isEmpty()) {
                    rolesMappingResource.realmLevel()
                            .add(rolesToAdd);
                }
                List<RoleRepresentation> rolesToRemove = existingRoles.stream()
                        .filter(
                                r -> r.startsWith(prefix) && !entry.getValue()
                                        .contains(r.substring(prefix.length()))
                        )
                        .map(this::ensureRole)
                        .toList();
    ...
    private RoleRepresentation ensureRole(String roleName) {
        try {
            ...
            keycloak.realm(realm)
                    .roles()
                    .create(new RoleRepresentation(roleName, null, false));
            return keycloak.realm(realm)
                    .roles()
                    .get(roleName)
                    .toRepresentation();
    ...
    public List<String> getAllTeams() {
        try {
            ...
            throw ServiceException.serverError(
                    "Please check with the System Administrators that you have the correct permissions"
            );
        }
    }
    ...
    public List<UserService.UserData> administrators() { // get the list of all the users with administrator role
        try {
            ...
            throw ServiceException.serverError(
                    "Please verify with the System Administrators that you have the correct permissions"
            );
        }
    }
    ...
    public void updateAdministrators(List<String> newAdmins) { // update the list of administrator users
        try {
            UsersResource usersResource = keycloak.realm(realm)
                    .users();
            RoleResource adminRoleResource = keycloak.realm(realm)
                    .roles()
                    .get(Roles.ADMIN);
                ...
                if (!newAdmins.contains(user.getUsername())) {
                    try {
                        usersResource.get(user.getId())
                                .roles()
                                .realmLevel()
                                .remove(List.of(adminRole));
                        LOG.infov("Removed administrator role from user {0}", user.getUsername());
                    } catch (Throwable t) {
                        LOG.warnv(
                                "Could not remove admin role from user {0} due to {1}",
                                user.getUsername(),
                                t.getMessage()
                        );
                    }
            ...
            for (String username : newAdmins) { // add admin role for `newAdmins` not in `oldAdmins`
                if (oldAdmins.stream()
                        .noneMatch(old -> username.equals(old.getUsername()))) {
                    try {
                        usersResource.get(findMatchingUserId(username))
                                .roles()
                                .realmLevel()
    ...
    public void setPassword(String username, String password) {
        try {
            ...
            keycloak.realm(realm)
                    .users()
                    .get(findMatchingUserId(username))
                    .resetPassword(credentials);
        } catch (Throwable t) {
            LOG.warnv(t, "Failed to retrieve current representation of user {0} from Keycloak", username);
            throw ServiceException.serverError(
                    format("Failed to retrieve current representation of user {0} from Keycloak", username)
            );
        }
    }
}
