...
public class HorreumAuthorizationFilter {
            ...
            if (payloadStart > 0 && payloadEnd > 0 && payloadStart < payloadEnd) {
                ...
                if (iss == null || iss.isBlank()) {
                    return Response.status(Response.Status.FORBIDDEN)
                            .entity("Authorization token does not contain issuer ('iss') claim.").build();
                }
                if (issuer.isPresent()) {
    ...
    private Response replyWrongIss(String iss, String expected) {
        return Response.status(Response.Status.FORBIDDEN).entity("Authorization token has issuer '" + iss
                + "' but this is not the expected issuer '" + expected
