...
public class RolesAugmentor implements SecurityIdentityAugmentor {
    ...
    @ConfigProperty(
            name = "horreum.roles.database.override",
            defaultValue = "true"
    )
    boolean override;
    ...
    public Uni<SecurityIdentity> augment(SecurityIdentity identity, AuthenticationRequestContext context) {
        return identity.isAnonymous() ?
                Uni.createFrom()
                        .item(identity) :
                context.runBlocking(() -> addHorreumRoles(identity));
    }
    ...
    private void addRoles(QuarkusSecurityIdentity.Builder builder, UserInfo user) {
        ...
        user.teams.stream()
                .map(TeamMembership::asRole)
                .forEach(builder::addRole);
        user.teams.stream()
                .map(TeamMembership::asTeam)
                .forEach(builder::addRole);
        user.teams.stream()
                .map(TeamMembership::asUIRole)
                .distinct()
                .forEach(builder::addRole);
    }
}
