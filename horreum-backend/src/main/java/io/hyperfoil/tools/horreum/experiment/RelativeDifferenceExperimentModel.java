...
public class RelativeDifferenceExperimentModel implements ExperimentConditionModel {
    ...
    @Override
    public ExperimentService.ComparisonResult compare(
            JsonNode config,
            List<DataPointDAO> baseline,
            DataPointDAO newDatapoint
    ) {
        int maxBaselineDatasets = config.get("maxBaselineDatasets")
                .asInt(0);
        Stream<DataPointDAO> stream = baseline.stream();
        if (maxBaselineDatasets > 0) {
            stream = stream.limit(maxBaselineDatasets);
        }
        OptionalDouble mean = stream.mapToDouble(dp -> dp.value)
                .average();
        if (mean.isEmpty()) {
        ...
        double threshold = config.get("threshold")
                .asDouble(0);
        boolean greaterBetter = config.get("greaterBetter")
                .asBoolean(true);
        ...
        return new ExperimentService.ComparisonResult(
                overall,
                newDatapoint.value,
                mean.getAsDouble(),
