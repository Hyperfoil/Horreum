...
public class LogServiceImpl implements LogService {
    ...
    @Override
    public List<DatasetLog> getDatasetLog(
            String source,
            int testId,
            int level,
            Integer datasetId,
        ...
        if (datasetId == null) {
            query = DatasetLogDAO.find(
                    "test.id = ?1 AND source = ?2 AND level >= ?3",
            ...
            query = DatasetLogDAO.find(
                    "dataset.id = ?1 AND source = ?2 AND level >= ?3",
                    Sort.descending("timestamp"),
                    datasetId,
                    source,
                    level
        ...
        return query.page(Page.of(page, limit))
                .list()
                .stream()
    ...
    public void deleteDatasetLogs(String source, int testId, Integer datasetId, Long from, Long to) {
        ...
        if (datasetId == null) {
            deleted = DatasetLogDAO.delete(
                    "test.id = ?1 AND source = ?2 AND timestamp >= ?3 AND timestamp < ?4",
                    testId,
                    source,
                    fromTs,
                    toTs
            ...
            deleted = DatasetLogDAO.delete(
                    "test.id = ?1 AND source = ?2 AND timestamp >= ?3 AND timestamp < ?4 AND dataset.id = ?5",
                    testId,
                    source,
                    fromTs,
                    toTs,
    ...
    @WithRoles
    @Override
    public List<TransformationLog> getTransformationLog(
            int testId,
            int level,
            Integer runId,
            Integer page,
        ...
        if (runId == null || runId <= 0) {
            List<TransformationLogDAO> logs = TransformationLogDAO.find(
                    "test.id = ?1 AND level >= ?2",
                    Sort.descending("timestamp"),
                    testId,
                    level
            )
                    ...
                    .list();
            return logs.stream()
                    .map(TransformationLogMapper::from)
                    .collect(Collectors.toList());
        } else {
            List<TransformationLogDAO> logs = TransformationLogDAO.find(
                    "test.id = ?1 AND level >= ?2 AND run.id = ?3",
                    Sort.descending("timestamp"),
                    testId,
                    level,
                    runId
    ...
    public void deleteTransformationLogs(int testId, Integer runId, Long from, Long to) {
        ...
        long deleted;
        if (runId == null || runId <= 0) {
            deleted = TransformationLogDAO.delete(
                    "test.id = ?1 AND timestamp >= ?2 AND timestamp < ?3",
                    testId,
                    fromTs,
                    toTs
            );
            ...
            deleted = TransformationLogDAO.delete(
                    "test.id = ?1 AND run.id = ?2 AND timestamp >= ?3 AND timestamp < ?4",
                    testId,
                    runId,
                    fromTs,
                    toTs
    ...
    public List<ActionLog> getActionLog(int testId, int level, Integer page, Integer limit) {
        page = withDefault(page, 0);
        limit = withDefault(limit, 25);
        List<ActionLogDAO> logs = ActionLogDAO.find(
                "testId = ?1 AND level >= ?2",
                Sort.descending("timestamp"),
                testId,
                level
        )
    ...
    public long getActionLogCount(int testId, int level) {
        return ActionLogDAO.find("testId = ?1 AND level >= ?2", testId, level)
                .count();
    }
    ...
    void checkExpiredTransformationLogs() {
        ...
        long logsDeleted = TransformationLogDAO.delete(
                "timestamp < ?1",
                timeService.now()
                        .minus(maxLifespan)
        );
