...
public class Util {
    ...
    public ObjectMapper producerObjectMapper() {
        ...
        ObjectMapper objectMapper = beanManager.getContext(bean.getScope())
    ...
    public static void doAfterCommitThrowing(TransactionManager tm, Runnable runnable) throws SystemException,
            RollbackException {
        tm.getTransaction()
    ...
    public static Runnable wrapForBlockingExecution(Runnable runnable) {
        ...
        Runnable withThreadContext = SmallRyeContextManagerProvider.getManager()
    ...
    public static Uni<Void> executeBlocking(
    ...
    * */
    static <T> void evaluateWithCombinationFunction(
            List<T> inputData,
        ...
        for (int i = 0; i < inputData.size(); i++) {
            ...
            if (jsFuncBody != null && !jsFuncBody.isBlank()) {
                try (
                        org.graalvm.polyglot.Context context = createContext(out)
                ) {
                    context.enter();
                    try {
                        setupContext(context);
                        StringBuilder jsCode = new StringBuilder("const __obj").append(i)
                                .append(" = ")
                                .append(evaluationInputObject.apply(element))
                                .append(";\n");
                        jsCode.append("const __func")
                                .append(i)
                                .append(" = ")
                                .append(jsFuncBody)
                                .append(";\n");
    ...
    private static void setupContext(Context context) throws IOException {
        context.getBindings("js")
                .putMember("isInstanceLike", new ProxyJacksonObject.InstanceCheck());
        context.eval(
                "js",
    ...
    public static Value resolvePromise(Value value) {
        if (value.getMetaObject()
                .getMetaSimpleName()
                .equals("Promise") && value.hasMember("then") && value.canInvokeMember("then")) {
    ...
    static <T> T evaluateOnce(
            String function,
            JsonNode input,
            Function<Value, T> processResult,
            BiConsumer<String, Throwable> onException,
            Consumer<String> onOutput
    ) {
        StringBuilder jsCode = new StringBuilder("const __obj = ").append(input)
                .append(";\n");
        jsCode.append("const __func = ")
                .append(function)
                .append(";\n");
        ...
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        try (
                Context context = createContext(out)
        ) {
            context.enter();
            try {
                setupContext(context);
    ...
    static boolean evaluateTest(
            String function,
            JsonNode input,
            Predicate<Value> onNotBoolean,
            BiConsumer<String, Throwable> onException,
            Consumer<String> onOutput
    ) {
    ...
    /**
     * returns true (in the CheckResult) if the input can be cast to the target type in psql, otherwise it is false and
    ...
    public static CheckResult castCheck(String input, String target, EntityManager em) {
        ...
        if (input != null && !input.isBlank()) {
            results = em.createNativeQuery(CHECK_CAST)
