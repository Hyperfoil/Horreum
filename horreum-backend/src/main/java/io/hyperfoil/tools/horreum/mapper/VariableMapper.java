...
public class VariableMapper {
    ...
    public static Variable from(VariableDAO dao) {
        ...
        v.changeDetection = dao.changeDetection.stream()
                .map(ChangeDetectionMapper::from)
                .collect(Collectors.toSet());
        //label
        if (dao.labels.isArray()) {
            v.labels = new ArrayList<>();
            dao.labels.spliterator()
                    .forEachRemaining(n -> {
                        LabelDAO l = LabelDAO.find("name", n.asText())
                                .firstResult();
                        if (l != null)
    ...
    public static VariableDAO to(Variable dto) {
        ...
        if (dto.changeDetection != null)
            v.changeDetection = dto.changeDetection.stream()
                    .map(ChangeDetectionMapper::to)
                    .collect(Collectors.toSet());
