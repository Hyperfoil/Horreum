...
public class HttpAction implements ActionPlugin {
    ...
    @ConfigProperty(
            name = "horreum.hook.tls.insecure",
            defaultValue = "false"
    )
    boolean insecureTls;

    @ConfigProperty(
            name = "horreum.hook.maxConnections",
            defaultValue = "20"
    )
    int maxConnections;
    ...
    public void postConstruct() {
        ...
        http1xClient = WebClient.create(
                reactiveVertx,
                new WebClientOptions(options).setProtocolVersion(HttpVersion.HTTP_1_1)
        );
    }
    ...
    public void validate(JsonNode config, JsonNode secrets) {
        String url = config.path("url")
                .asText();
        if (AllowedSiteDAO.find("?1 LIKE CONCAT(prefix, '%')", url)
                .count() == 0) {
            throw ServiceException.badRequest(
                    "The requested URL is not on the list of allowed URL prefixes; " +
    ...
    public Uni<String> execute(JsonNode config, JsonNode secrets, Object payload) {
        String urlPattern = config.path("url")
                .asText();
        if (urlPattern == null) {
                ...
                .transform(response -> {
                    if (response.statusCode() < 400) {
                        ...
                        throw new IllegalArgumentException(
                                "Failed to POST " + url + ", response " + response.statusCode() + ": " + response
                                        .bodyAsString()
                        );
                    }
                })
                .onFailure()
                .transform(t -> new RuntimeException("Failed to POST " + url + ": " + t.getMessage()));
    }
}
