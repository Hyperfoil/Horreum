...
public class ExperimentServiceImpl implements ExperimentService {
    ...
    private static final Map<String, ExperimentConditionModel> MODELS = Map.of(
            RelativeDifferenceExperimentModel.NAME,
            new RelativeDifferenceExperimentModel()
    );
    ...
    public Collection<ExperimentProfile> profiles(int testId) {
        ...
        return profiles.stream()
                .map(ExperimentProfileMapper::from)
    ...
    public List<ConditionConfig> models() {
        return MODELS.values()
                .stream()
                .map(ExperimentConditionModel::config)
                .collect(Collectors.toList());
    }
    ...
    public void onDatapointsCreated(DataPoint.DatasetProcessedEvent event) {
        ...
        runExperiments(
                event.dataset,
                result -> Util.registerTxSynchronization(
                        tm,
                        value -> mediator.publishEvent(
                                AsyncEventChannels.EXPERIMENT_RESULT_NEW,
                                event.dataset.testId,
                                result
                        )
                ),
    ...
    private void runExperiments(
            Dataset.Info info,
            Consumer<ExperimentResult> resultConsumer,
            Consumer<List<DatasetLogDAO>> noProfileConsumer,
            boolean notify
    ) {
        List<DatasetLogDAO> logs = new ArrayList<>();

        NativeQuery<Object[]> selectorQuery = em.unwrap(Session.class)
                .createNativeQuery(
                        """
                                WITH lvalues AS (
        ...
        List<Object[]> selectorRows = selectorQuery.setParameter(1, info.testId)
                .setParameter(2, info.id)
        ...
        Util.evaluateWithCombinationFunction(
                ...
                (r, ex, code) -> addLog(
                        ...
                        "Selector filter failed: %s Code: %s",
                        ex.getMessage(),
                        code
                ),
                output -> addLog(
        ...
        NativeQuery<Object[]> baselineQuery = em.unwrap(Session.class)
                .createNativeQuery(
                        """
                                WITH lvalues AS (
        ...
        List<Object[]> baselineRows = baselineQuery.setParameter(1, matchingProfile)
                .setParameter(2, info.testId)
                .addScalar("profile_id", StandardBasicTypes.INTEGER)
        ...
        Util.evaluateWithCombinationFunction(
                ...
                (r, v) -> {
                    if (v.asBoolean()) {
                        baselines.computeIfAbsent((Integer) r[0], profileId -> new ArrayList<>())
                                .add((Integer) r[3]);
                    }
                },
                r -> {
                ...
                (r, ex, code) -> addLog(
                        perProfileLogs.get((Integer) r[0]),
                        info.testId,
                        (Integer) r[3],
                        PersistentLogDAO.ERROR,
                        "Baseline filter failed: %s Code: %s",
                        ex.getMessage(),
                        code
                ),
                output -> perProfileLogs.forEach(
                        (profileId, pls) -> addLog(
                                pls,
                                info.testId,
                                info.id,
                                PersistentLogDAO.DEBUG,
                                "Baseline filter output: %s",
                                output
                        )
                )
        );
        ...
        for (var entry : baselines.entrySet()) {
            ...
            List<Integer> variableIds = profile.comparisons.stream()
                    .map(ExperimentComparisonDAO::getVariableId)
                    .collect(Collectors.toList());
            DataPointDAO.<DataPointDAO>find(
                    "dataset.id IN ?1 AND variable.id IN ?2",
                    Sort.descending("timestamp", "dataset.id"),
                    entry.getValue(),
                    variableIds
            )
                    .stream()
                    .forEach(
                            dp -> byVar.computeIfAbsent(dp.variable.id, v -> new ArrayList<>())
                                    .add(dp)
                    );
            ...
            for (var comparison : profile.comparisons) {
                ...
                if (model == null) {
                    addLog(
                            ...
                            info.id,
                            PersistentLogDAO.ERROR,
                            "Unknown experiment comparison model '%s' for variable %s in profile %s",
                            comparison.model,
                ...
                if (datapoint == null) {
                    addLog(
                            ...
                            info.id,
                            PersistentLogDAO.ERROR,
                            "No datapoint for comparison of variable %s in profile %s",
                            comparison.variable.name,
                            profile.name
                    );
                    continue;
                ...
                results.put(
                        ExperimentProfileMapper.fromExperimentComparison(comparison),
                        model.compare(comparison.config, baseline, datapoint)
                );
            }

            org.hibernate.query.Query<Dataset.Info> datasetQuery = em.unwrap(Session.class)
                    .createQuery(
                            "SELECT id, run.id, ordinal, testid FROM dataset WHERE id IN ?1 ORDER BY start DESC",
                            Dataset.Info.class
                    );
            datasetQuery.setTupleTransformer(
                    (tuples, aliases) -> new Dataset.Info(
                            (int) tuples[0],
                            (int) tuples[1],
                            (int) tuples[2],
                            (int) tuples[3]
            ...
            List<Dataset.Info> baseline = datasetQuery.setParameter(1, entry.getValue())
                    .getResultList();
            ...
            ExperimentResult result = new ExperimentResult(
                    ExperimentProfileMapper.from(profile),
                    profileLogs.stream()
                            .map(DatasetLogMapper::from)
                            .collect(Collectors.toList()),
                    info,
