...
public class RunMapper {
    ...
    public static Run from(RunDAO run) {
        ...
        if (run.validationErrors != null)
            dto.validationErrors = run.validationErrors.stream()
                    .map(ValidationErrorMapper::fromValidationError)
                    .collect(Collectors.toList());
        if (run.datasets != null)
            dto.datasets = run.datasets.stream()
                    .map(DatasetMapper::from)
                    .collect(Collectors.toList());
        dto.owner = run.owner;
    ...
    public static RunDAO to(Run dto) {
        ...
        if (dto.validationErrors != null) {
            run.validationErrors = dto.validationErrors.stream()
                    .map(ValidationErrorMapper::toValidationError)
                    .collect(Collectors.toList());
        }
        if (dto.datasets != null) {
            run.datasets = dto.datasets.stream()
                    .map(dsDTO -> DatasetMapper.to(dsDTO, run))
                    .collect(Collectors.toList());
        }
