...
public class TestMapper {
    public static Test from(TestDAO t) {
        ...
        if (t.tokens != null)
            dto.tokens = t.tokens.stream()
                    .map(TestMapper::fromTestToken)
                    .collect(Collectors.toList());
        if (t.transformers != null)
            dto.transformers = t.transformers.stream()
    ...
    public static TestDAO to(Test dto) {
        ...
        if (dto.tokens != null)
            t.tokens = dto.tokens.stream()
                    .map(token -> TestMapper.toTestToken(token, t))
                    .collect(Collectors.toList());
        t.views = ViewDAO.<ViewDAO>find("test.id", dto.id)
                .list();
        if (dto.transformers != null)
            t.transformers = dto.transformers.stream()
                    .map(TransformerMapper::to)
                    .collect(Collectors.toList());
