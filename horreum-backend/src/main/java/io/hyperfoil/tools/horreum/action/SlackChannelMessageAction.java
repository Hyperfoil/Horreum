...
public class SlackChannelMessageAction extends SlackPluginBase implements ActionPlugin {
    ...
    public Uni<String> execute(JsonNode config, JsonNode secrets, Object payload) {
        ...
        String channel = replaceExpressions(
                config.path("channel")
                        .asText(),
                json
        );
        String url = ConfigProvider.getConfig()
                .getValue("horreum.action.slack.url", String.class);
                ...
                .transformToUni(response -> {
                    if (response.statusCode() < 400) {
                            ...
                            return Uni.createFrom()
                                    .failure(
                                            new RuntimeException(
                                                    String.format(
                                                            "Failed to post to channel %s, response %s",
                                                            channel,
                        ...
                        return Uni.createFrom()
                                .item(
                                        String.format(
                                                "Successfully(%d) posted to channel %s",
                                                response.statusCode(),
                                                channel
                                        )
                                );
                    } else if (response.statusCode() == HttpStatus.SC_TOO_MANY_REQUESTS && response.getHeader(
                            "Retry-After"
                    ) != null) {
                        ...
                        return Uni.createFrom()
                                .failure(
                                        new RuntimeException(
                                                String.format(
                                                        "Failed to post to channel %s, response %d: %s",
                                                        channel,
                                                        response.statusCode(),
