...
public class EmailPlugin implements NotificationPlugin {
    ...
    @ConfigProperty(
            name = "horreum.mail.subject.prefix",
            defaultValue = "[Horreum]"
    )
    String subjectPrefix;

    @ConfigProperty(
            name = "horreum.mail.timeout",
            defaultValue = "15s"
    )
    ...
    public class EmailNotification extends Notification {
        ...
        public void notifyChanges(DatasetChanges event) {
            ...
            String content = changeNotificationEmail
                    ...
                    .data(
                            "fingerprint",
                            URLEncoder.encode(
                                    event.fingerprint != null ? event.fingerprint : "",
        ...
        @Override
        public void notifyMissingDataset(
                String testName,
                int testId,
                String ruleName,
                long maxStaleness,
        ...
        public void notifyMissingValues(String testName, String fingerprint, MissingValuesEvent event) {
            String subject = String.format(
                    "%s Missing change detection values in test %s, dataset %d#%d",
                    subjectPrefix,
                    testName,
                    event.dataset.runId,
                    event.dataset.ordinal + 1
            );
