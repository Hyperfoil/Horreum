...
public class GitHubIssueCommentAction extends GitHubPluginBase implements ActionPlugin {
    ...
    public Uni<String> execute(JsonNode config, JsonNode secrets, Object payload) {
        ...
        String token = secrets.path("token")
                .asText();
        if (token == null || token.isBlank()) {
            throw new IllegalArgumentException("Missing access token!");
        }
        String formatter = replaceExpressions(
        ...
        String issueUrl = replaceExpressions(
                config.path("issueUrl")
                        .asText(),
                json
        );
        String path;
        if (issueUrl != null) {
            ...
            String owner = replaceExpressions(
                    config.path("owner")
                            .asText(),
                    json
            );
            String repo = replaceExpressions(
                    config.path("repo")
                            .asText(),
                    json
            );
            String issue = replaceExpressions(
                    config.path("issue")
                            .asText(),
                    json
            );
        ...
        return post(
                ...
                .transformToUni(response -> {
                    if (response.statusCode() < 400) {
                        return Uni.createFrom()
                                .item(
                                        String.format(
                                                "Successfully(%d) added comment to %s",
                                                response.statusCode(),
                                                path
                                        )
                                );
                        ...
                        return Uni.createFrom()
                                .failure(
                                        new RuntimeException(
                                                String.format(
                                                        "Failed to add comment to %s, response %d: %s",
                                                        path,
                                                        response.statusCode(),
                ...
                .onFailure()
                .transform(t -> new RuntimeException("Failed to add comment to " + path + ": " + t.getMessage()));
    }
}
