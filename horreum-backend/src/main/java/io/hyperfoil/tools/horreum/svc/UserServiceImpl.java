...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<String> getRoles() { return identity.getRoles()
            .stream()
            .toList(); }
    ...
    public void createUser(NewUser user) {
        ...
        Log.infov(
                "{0} created user {1} {2} with username {3} on team {4}",
    ...
    @Override
    public List<String> getTeams() { return backend.get()
            .getTeams(); }
    ...
    public String defaultTeam() {
        UserInfo userInfo = UserInfo.findById(
                identity.getPrincipal()
                        .getName()
        );
        if (userInfo == null) {
    ...
    public Map<String, List<String>> teamMembers(String unsafeTeam) {
        ...
        Map<String, List<String>> teamMembers = backend.get()
                .teamMembers(team);
    ...
    public void updateTeamMembers(String unsafeTeam, Map<String, List<String>> newRoles) {
        ...
        backend.get()
                .teamMembers(team)
                .forEach((username, old) -> roles.putIfAbsent(username, emptyList()));
        safeMachineAccounts(team).forEach(roles::remove); // exclude machine accounts

        backend.get()
                .updateTeamMembers(team, roles);
    }

    @RolesAllowed(Roles.ADMIN)
    @Override
    public List<String> getAllTeams() { return backend.get()
            .getAllTeams(); }
    ...
    public void updateAdministrators(List<String> newAdmins) {
        ...
        backend.get()
                .updateAdministrators(newAdmins);
    }
    ...
    public String resetPassword(String unsafeTeam, String username) {
        ...
        if (backend.get()
                .machineAccounts(team)
                .stream()
                .noneMatch(data -> data.username.equals(username))) {
    ...
    private void userIsManagerForTeam(String team) {
        if (!identity.getRoles()
                .contains(Roles.ADMIN) && !identity.hasRole(team.substring(0, team.length() - 4) + Roles.MANAGER)) {
    ...
    void createLocalUser(String username, String defaultTeam, String password) {
        UserInfo userInfo = UserInfo.<UserInfo>findByIdOptional(username)
                .orElse(new UserInfo(username));
        if (defaultTeam != null) {
