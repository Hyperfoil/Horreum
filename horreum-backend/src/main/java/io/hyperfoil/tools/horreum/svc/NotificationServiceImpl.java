...
public class NotificationServiceImpl implements NotificationService {
   ...
   //@formatter:on
    ...
    public void onNewChanges(DatasetChanges event) {
        ...
        log.debugf(
                "Received new changes in test %d (%s), dataset %d/%d (fingerprint: %s)",
                event.dataset.testId,
                event.testName,
                event.dataset.runId,
                event.dataset.ordinal,
                event.fingerprint
        );
    ...
    public void onMissingValues(MissingValuesEvent event) {
        if (!event.notify) {
            log.debugf(
                    "Skipping notification for missing run values on test %d, run %d",
                    event.dataset.testId,
                    event.dataset.id
            );
            return;
        ...
        log.debugf(
                "Received missing values event in test %d (%s), run %d, variables %s",
                event.dataset.testId,
                testName,
                event.dataset.id,
                event.variables
        ...
        String fingerprint = em.getReference(DatasetDAO.class, event.dataset.id)
                .getFingerprint();
    ...
    private void notifyAll(int testId, Consumer<Notification> consumer) {
        List<Object[]> results = em.unwrap(Session.class)
                .createNativeQuery(GET_NOTIFICATIONS, Object[].class)
    ...
    @RolesAllowed({
            Roles.VIEWER, Roles.TESTER, Roles.ADMIN
    })
    @Override
    public List<NotificationSettings> settings(String name, boolean team) {
        List<NotificationSettingsDAO> notifications = NotificationSettingsDAO.list(
                "name = ?1 AND isTeam = ?2",
                name,
                team
        );
        return notifications.stream()
                .map(NotificationSettingsMapper::from)
    ...
    public void updateSettings(String name, boolean team, NotificationSettings[] settings) {
        ...
        Arrays.stream(settings)
                .map(NotificationSettingsMapper::to)
                .forEach(ns -> {
