...
public class SubscriptionServiceImpl implements SubscriptionService {
    ...
    public Watch get(int testId) {
        WatchDAO watch = WatchDAO.find("test.id = ?1", testId)
                .firstResult();
        if (watch == null) {
            watch = new WatchDAO();
    ...
    public List<String> addUserOrTeam(int testId, String userOrTeam) {
        ...
        if (userOrTeam.equals("__self") || userOrTeam.equals(username)) {
            userOrTeam = username;
            isTeam = false;
        } else if (!userOrTeam.endsWith("-team") || !identity.getRoles()
                .contains(userOrTeam)) {
        ...
        WatchDAO watch = WatchDAO.find("test.id", testId)
                .firstResult();
        if (watch == null) {
            watch = new WatchDAO();
            watch.test = em.getReference(TestDAO.class, testId);
    ...
    public void update(int testId, Watch dto) {
        ...
        WatchDAO existing = WatchDAO.find("test.id", testId)
                .firstResult();
        if (existing == null) {
            watch.id = null;
            watch.test = em.getReference(TestDAO.class, testId);
