...
public class ChangesServiceImpl implements ChangesService {
    ...
    void init() {
        ...
        if (baseUrl.contains("//localhost")) {
            ...
            int devServerPort = config.getOptionalValue("quarkus.quinoa.dev-server.port", int.class)
                    .orElse(-1);
            if (devServerPort > 0) {
                allowedOrigins.add(
                        baseUrl.substring(0, colonIndex < 0 ? baseUrl.length() : colonIndex) + ":" + devServerPort
                );
            }
        }
        config.getOptionalValue("horreum.internal.url", String.class)
                .ifPresent(url -> allowedOrigins.add(getOrigin(url)));
    }
    ...
    public String[] search(Target query) {
        return VariableDAO.<VariableDAO>listAll()
                .stream()
                .map(v -> String.valueOf(v.id))
                .toArray(String[]::new);
    }
    ...
    public List<TimeseriesTarget> query(Query query) {
        ...
        for (Target target : query.targets) {
            ...
            if (query.range.oneBeforeAndAfter) {
                sql.append(
                        "UNION (SELECT * FROM datapoint WHERE variable_id = ?1 AND timestamp < ?2 ORDER BY timestamp DESC LIMIT 1) "
                )
                        .append(
                                "UNION (SELECT * FROM datapoint WHERE variable_id = ?1 AND timestamp > ?3 ORDER BY timestamp LIMIT 1)"
                        );
            }
            sql.append(") SELECT dp.* FROM dp ");
            if (fingerprint != null) {
                sql.append(
                        "LEFT JOIN fingerprint fp ON fp.dataset_id = dp.dataset_id WHERE json_equals(fp.fingerprint, (?4)::jsonb) "
                );
            }
            sql.append("ORDER BY timestamp ASC");
            NativeQuery<DataPointDAO> nativeQuery = em.unwrap(Session.class)
                    .createNativeQuery(sql.toString(), DataPointDAO.class)
            ...
            for (DataPointDAO dp : datapoints) {
                tt.datapoints.add(new Number[]{
                        dp.value, dp.timestamp.toEpochMilli(), /* non-standard! */ dp.dataset.id
                });
            }
        }
        return result;
    ...
    @Override
    public Response annotations(@HeaderParam("Origin")
    String origin) {
        ...
        for (String allowed : allowedOrigins) {
            if (allowed.equals(origin)) {
                return response.header("Access-Control-Allow-Origin", allowed)
                        .build();
            }
        }
        return response.header("Access-Control-Allow-Origin", allowedOrigins.get(0))
                .build();
    }
    ...
    public List<AnnotationDefinition> annotations(AnnotationsQuery query) {
        ...
        NativeQuery<ChangeDAO> nativeQuery = em.unwrap(Session.class)
                .createNativeQuery(sql.toString(), ChangeDAO.class)
                .setParameter(1, variableId)
                .setParameter(2, query.range.from)
                .setParameter(3, query.range.to);
        if (fingerprint != null) {
    ...
    private AnnotationDefinition createAnnotation(ChangeDAO change) {
        ...
        if (change.variable.group != null) {
            content.append(" (group ")
                    .append(change.variable.group)
                    .append(")");
        }
        content.append("<br>")
                .append(change.description)
                .append("<br>Confirmed: ")
                .append(change.confirmed);
        return new AnnotationDefinition(
                "Change in run " + change.dataset.run.id + "/" + change.dataset.ordinal,
                content.toString(),
                false,
                change.timestamp.toEpochMilli(),
                0,
                new String[0],
                change.id,
                change.variable.id,
                change.dataset.run.id,
