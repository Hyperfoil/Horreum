...
public class TestServiceImpl implements TestService {
   ...
   //@formatter:on
    ...
    public Test get(int id, String token) {
        TestDAO test = TestDAO.find("id", id)
                .firstResult();
        if (test == null) {
    ...
    public Test getByNameOrId(String input) {
        ...
        if (input.matches("-?\\d+")) {
            ...
            test = TestDAO.find("name = ?1 or id = ?2", input, id)
                    .firstResult();
        }
        if (test == null) {
            test = TestDAO.find("name", input)
                    .firstResult();
        }
        if (test == null) {
    ...
    public TestDAO ensureTestExists(String testNameOrId, String token) {
        ...
        if (testNameOrId.matches("-?\\d+")) {
            ...
            test = TestDAO.find("name = ?1 or id = ?2", testNameOrId, id)
                    .firstResult();
        } else {
            test = TestDAO.find("name", testNameOrId)
                    .firstResult();
        }
        if (test != null) {// we won't return the whole entity with any data
            ...
            if (Roles.hasRoleWithSuffix(identity, test.owner, "-uploader")) {
                return detached;
            } else if (token != null && test.tokens.stream()
                    .anyMatch(tt -> tt.valueEquals(token) && tt.hasUpload())) {
                        return detached;
                    }
            log.debugf(
                    "Failed to retrieve test %s as this user (%s = %s) is not uploader for %s and token %s does not match",
                    testNameOrId,
    ...
    TestDAO addAuthenticated(Test dto) {
        TestDAO existing = TestDAO.find("id", dto.id)
                .firstResult();
        if (existing == null)
            dto.clearIds();
    ...
    private void verifyTransformersBeforeAdd(TestDAO test) {
        ...
        for (var t : test.transformers) {
            if (TransformerDAO.findById(t.id) == null) {
                TransformerDAO trans = TransformerDAO.find("targetSchemaUri", t.targetSchemaUri)
                        .firstResult();
                if (trans != null)
                    tmp.add(trans);
            } else
    ...
    public TestListing summary(
        ...
        testSql.append(
                "FROM test LEFT JOIN runs ON runs.testid = test.id LEFT JOIN datasets ON datasets.testid = test.id"
        );
        ...
        org.hibernate.query.Query<TestSummary> testQuery = em.unwrap(Session.class)
                ...
                .setTupleTransformer(
                        (tuples, aliases) -> new TestSummary(
                                (int) tuples[0],
                                (String) tuples[1],
                                (String) tuples[2],
                                (String) tuples[3],
        ...
        if (!identity.isAnonymous()) {
            ...
            List<WatchDAO> subscriptions = em.createNativeQuery(
                    "SELECT * FROM watch w WHERE w.testid IN (?1)",
                    WatchDAO.class
            )
                    .setParameter(1, testIdSet)
                    .getResultList();
            String username = identity.getPrincipal()
                    .getName();
            Set<String> teams = identity.getRoles()
                    .stream()
                    .filter(role -> role.endsWith("-team"))
                    .collect(Collectors.toSet());

            subscriptions.forEach(subscription -> {
                Set<String> subscriptionSet = subscriptionMap.computeIfAbsent(
            ...
            summaryList.forEach(
                    summary -> summary.watching = subscriptionMap.computeIfAbsent(
                            summary.id,
    ...
    public List<String> folders(String roles) {
        ...
        NativeQuery<String> query = em.unwrap(Session.class)
    ...
    public Collection<TestToken> tokens(int testId) {
        ...
        return t.tokens.stream()
                .map(TestTokenMapper::from)
    ...
    /**
     * returns true if the jsonpath input appears to be a predicate jsonpath (always returns true or false) versus a
     * filtering path (returns null or a value)
     *
     * @param input
     * @return
     */
    ...
    protected static FilterDef getFilterDef(
            String filter,
            Instant before,
            Instant after,
            boolean multiFilter,
                ...
                objectNode.fields()
                        .forEachRemaining(e -> {
                            ...
                            if (value.getNodeType() == JsonNodeType.ARRAY) {//check if there are any matches
                                ObjectNode arrayFilter = (ObjectNode) new JsonNodeFactory(false).objectNode()
                                        .set(key, value);
...
//                  List<ExportedLabelValues> found = labelValues(testId, arrayFilter.toString(), before, after, filtering, metrics, sort, direction, limit, page, include, exclude, false);
    ...
    public List<ExportedLabelValues> labelValues(
        ...
        FilterDef filterDef = getFilterDef(
                filter,
                beforeInstant,
                afterInstant,
                multiFilter,
        ...
        if ("start".equalsIgnoreCase(sort)) {
            ...
            if (!sort.isBlank()) {
                ...
                if (jsonpathResult.ok()) {
                    orderSql = LABEL_ORDER_PREFIX + LABEL_ORDER_JSONPATH + " " + orderDirection +
                            ", combined.runId DESC";
                }
            }
    ...
    public List<ObjectNode> filteringLabelValues(
        ...
        TestDAO.findByIdOptional(testId)
                .orElseThrow(() -> ServiceException.serverError("Cannot find test " + testId));
    ...
    public void recalculateDatasets(int testId) {
        ...
        while (prev != null) {
            ...
            if (prev.timestamp < System.currentTimeMillis() - TimeUnit.MINUTES.toMillis(10)) {
                log.warnf(
                        "Recalculation for test %d (%s) from %s timed out after 10 minutes with %d/%d runs",
                        testId,
                        test.name,
                        Instant.ofEpochMilli(prev.timestamp),
                        prev.finished,
                        prev.totalRuns
                );
        ...
        ScrollableResults results = em.createNativeQuery(
                "SELECT id FROM run WHERE testid = ?1 AND NOT trashed ORDER BY start"
        )
                .setParameter(1, testId)
                .unwrap(NativeQuery.class)
                .setReadOnly(true)
        ...
        while (results.next()) {
            ...
            log.debugf(
                    "Recalculate Datasets for run %d - forcing recalculation for test %d (%s)",
                    runId,
                    testId,
                    test.name
            );
...
//               mediator.queueRunRecalculation(runId);
    ...
    protected TestDAO getTestForUpdate(int testId) {
        ...
        if (!identity.hasRole(test.owner) || !identity.hasRole(
                test.owner.substring(0, test.owner.length() - 4) + Roles.TESTER
        )) {
