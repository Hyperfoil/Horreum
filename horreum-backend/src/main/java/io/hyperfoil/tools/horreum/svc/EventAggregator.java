...
public class EventAggregator {
    ...
    public synchronized void onNewChange(Change.Event event) {
        datasetChanges.computeIfAbsent(event.dataset.id, id -> {
            String fingerprint = DatasetDAO.getEntityManager()
                    .getReference(DatasetDAO.class, event.dataset.id)
                    .getFingerprint();
    ...
    void handleDatasetChanges() {
        ...
        while (true) {
            DatasetChanges next = this.datasetChanges.values()
                    .stream()
                    .reduce((dc1, dc2) -> dc1.emitTimestamp() < dc2.emitTimestamp() ? dc1 : dc2)
                    .orElse(null);
            if (next == null) {
                return;
