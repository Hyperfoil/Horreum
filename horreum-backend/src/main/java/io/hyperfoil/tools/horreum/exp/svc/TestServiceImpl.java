...
public class TestServiceImpl implements TestService {
    ...
    public Test createRhivos() {
        ...
        LabelGroupDao datasetGroup = new LabelGroupDao("urn:rhivos-perf-comprehensive-datasets:01");
        t.setLabels(
                new LabelDao(transformName, t)
                        .loadExtractors(
                                ...
                                ExtractorDao.fromString("$.stressng_workload[*].test_results.test_config.stressors[0].workers")
                                        .setName("workers"),
                                ExtractorDao.fromString("$.stressng_workload[*].test_results.test_config.stressors[0].stressor")
                                        .setName("stressor"),
                                ...
                                ExtractorDao.fromString("$.coremark_pro_workload.pcp_time_series")
                                        .setName("coremark_pro_pcp_ts"),
                                ExtractorDao.fromString("$.coremark_pro_workload[*].sample_uuid")
                                        .setName("coremark_pro_sample_uuid"),
                ...
                new LabelDao("CoreMark-PRO Multi Core", t)
                        .loadExtractors(
                                ExtractorDao.fromString(transformPrefix + "$.results.\"CoreMark-PRO\".MultiCore")
                                        .setName("coremark-pro-multi-core")),
                new LabelDao("CoreMark-PRO Scaling", t)
                        .loadExtractors(
                                ExtractorDao.fromString(transformPrefix + "$.results.\"CoreMark-PRO\".Scaling")
                                        .setName("coremark-pro-scaling")),
                new LabelDao("CoreMark-PRO Single Core", t)
                        .loadExtractors(
                                ExtractorDao.fromString(transformPrefix + "$.results.\"CoreMark-PRO\".SingleCore")
                                        .setName("coremark-pro-single-core")),
                ...
                new LabelDao("Hostname", t)
                        .loadExtractors(
                                ExtractorDao.fromString(transformPrefix + "$.metadata.ansible_facts.env.HOSTNAME")
                                        .setName("Hostname")),
    ...
    public List<LabelService.ValueMap> labelValues(
        ...
        if (group != null && !group.isBlank()) {
            LabelGroupDao labelGroup = LabelGroupDao.find("name", group).firstResult();
            //TODO call labelValues with schema
            return service.labelValues(labelGroup, testId, include, exclude);
        } else {
            return service.labelValues(testId, filter, before, after, sort, direction, limit, page, include, exclude,
                    multiFilter);
        }
    }
}
