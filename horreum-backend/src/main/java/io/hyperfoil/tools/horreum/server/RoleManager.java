...
public class RoleManager {
    ...
    String setRoles(String roles) {
        ...
        Object[] row = (Object[]) em.createNativeQuery(SET_ROLES)
        ...
        if (Log.isDebugEnabled()) { // enabe with: `quarkus.log.category."io.hyperfoil.tools.horreum.server.RoleManager".level=DEBUG`
            try {
                Log.debugv(
                        "Setting roles {0} (replacing {1}) on transaction {2}",
                        roles,
                        row[0],
                ...
                Log.debugv(
                        "Setting roles {0} (replacing {1}), but obtaining current transaction failed due to {2}",
                        roles,
                        row[0],
                        e.getMessage()
                );
