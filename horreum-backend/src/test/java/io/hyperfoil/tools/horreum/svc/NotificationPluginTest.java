...
public class NotificationPluginTest {
    ...
    public void testDatasetChanges() {
        DatasetChanges dc1 = new DatasetChanges(
                new Dataset.Info(1, 1, 0, 1),
                "{\"runtime\": \"native\"}",
                "Dummy Test",
                true
        );
    ...
    public void testExpectedRun() {
        withAllPlugins(
                notification -> notification.notifyExpectedRun(
                        "Dummy Test",
                        1,
    ...
    public void testMissingDataset() {
        withAllPlugins(
                notification -> notification.notifyMissingDataset(
                        "Dummy Test",
                        1,
                        "My rule",
    ...
    @Test
    public void test() {
        var event = new MissingValuesEvent(
                new DatasetDAO.Info(1, 1, 0, 1),
    ...
    private void withAllPlugins(Consumer<Notification> consumer) {
        plugins.stream()
                .forEach(p -> consumer.accept(p.create("dummy", "dummy@example.com")));
    }
}
