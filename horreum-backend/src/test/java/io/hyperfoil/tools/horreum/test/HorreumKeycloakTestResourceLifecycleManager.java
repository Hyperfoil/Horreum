...
/**
 * Extends {@link KeycloakTestResourceLifecycleManager} so that the service client has <pre>realm-admin</pre> role to be
 * able to manage users in the realm
 */
public class HorreumKeycloakTestResourceLifecycleManager extends KeycloakTestResourceLifecycleManager {
    ...
    private static final String KEYCLOAK_SERVICE_CLIENT = System.getProperty(
    ...
    public Map<String, String> start() {
        ...
        try (
            ...
            String clientId = realmResource.clients()
                    .findByClientId(KEYCLOAK_SERVICE_CLIENT)
                    .get(0)
                    .getId();
            String managementId = realmResource.clients()
                    .findByClientId("realm-management")
                    .get(0)
                    .getId();
            String serviceAccountId = realmResource.clients()
                    .get(clientId)
                    .getServiceAccountUser()
                    .getId();

            RoleScopeResource managementRolesResource = realmResource.users()
                    .get(serviceAccountId)
                    .roles()
                    .clientLevel(managementId);
            managementRolesResource.add(
                    managementRolesResource.listAvailable()
                            .stream()
                            .filter(r -> "realm-admin".equals(r.getName()))
                            .toList()
            );
