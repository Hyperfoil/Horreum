...
public class UtilTest {
    ...
    private static class CloseableServer implements Closeable {
        ...
        public CloseableServer(String endpoint, String response) throws IOException {
            httpServer = HttpServer.create(
                    ...
                    0
            ); // or use InetSocketAddress(0) for ephemeral port
    ...
    public void toInstant_nulls() throws UnsupportedEncodingException {
        try (
            ...
            assertFalse(
                    handler.getLog()
                            .contains("DateTimeParseException"),
                    handler.getLog()
            );
        }
    }
    ...
    public void toInstant_valid() throws IOException {
        ...
        assertNotNull(
                Util.toInstant("2024-03-11T22:16:24.302655-04:00"),
                "failed to parse ISO with microseconds and zzzz"
        );
        //json
        assertNotNull(Util.toInstant(new TextNode("2020-01-01")), "failed to parse json text YYYY-MM-DD");
        assertNotNull(
                Util.toInstant(new LongNode(System.currentTimeMillis())),
                "failed to parse current millis as json node"
        );
    }
    ...
    public void evaluateOnceJsonKeyAccess() throws JsonProcessingException {
        ...
        Assertions.assertInstanceOf(
                String.class,
                rtrn,
                "return should be a string but was " + (rtrn.getClass()
                        .getSimpleName())
        );
    ...
    public void evaluateOnceAsyncAwaitFetchJson() {
        try (
                CloseableServer server = new CloseableServer("/", "{\"key\":42}")
        ) {
            Object rtrn = Util.evaluateOnce(
                    """
                            async ()=>{
            ...
            Assertions.assertEquals(
                    0,
                    server.getCallCount(),
                    "request should not call service due to context sandboxing"
            );
