...
public class TokenAuthTest extends BaseServiceTest {
    ...
    public void testTokenInHeader() {
        ...
        // assert test exists
        jsonRequest().get("/api/test/" + test.id)
                .then()
                .statusCode(200);
        // assert not accessible without auth
        given().get("/api/test/" + test.id)
                .then()
                .statusCode(404);
        ...
        given().get("/api/test/" + test.id)
                .then()
                .statusCode(404);
        given().get("/api/test/" + test.id + "?token=foobar")
                .then()
                .statusCode(200);
        given().get("/api/test/" + test.id + "?token=blabla")
                .then()
                .statusCode(404);

        given().header(TokenInterceptor.TOKEN_HEADER, "foobar")
                .get("/api/test/" + test.id)
                .then()
                .statusCode(200);
        given().header(HttpHeaders.AUTHORIZATION, "Bearer foobar")
                .get("/api/test/" + test.id)
                .then()
                .statusCode(200);

        given().get("/api/test/" + test.id + "?token=" + getAccessToken("alice", TESTER_ROLES))
                .then()
                .statusCode(400);
    }
}
