    ...
    public Response addRunFromData(
        ...
        if (metadata != null && metadata.length > 0) {
            ...
            if (metadata.length > 1) {
                if (Stream.of(metadata)
                        .anyMatch(obj -> !obj.isObject())) {
                    throw new IllegalArgumentException("When passing multiple metadata each item must be an object.");
                } else if (Stream.of(metadata)
                        .anyMatch(obj -> !obj.hasNonNull("$schema"))) {
                            throw new IllegalArgumentException("All metadata must have '$schema' set.");
                        }
                combinedMetadata = JsonNodeFactory.instance.arrayNode()
                        .addAll(Arrays.asList(metadata));
            } else {
