...
public class RunService {
   ...
   @POST
   @Path("list/{testId}/")
   public Response testList(@PathParam("testId") Integer testId, Json options){
      Test test = Test.find("id",testId).firstResult();
      if(test==null){
         return Response.serverError().entity("failed to find test "+testId).build();
      }
      ...
      Integer limit = (int)options.getLong("limit",-1);
      Integer page = (int)options.getLong("page",-1);
      String sort = options.getString("sort",null);
      String direction = options.getString("direction",null);
