...
public class HorreumResources {
    ...
    static {
        ...
        try (
                InputStream propertyStream = classLoader.getResourceAsStream("env.properties");
    ...
    public static Map<String, String> startContainers(Map<String, String> initArgs) {
        ...
        if (Boolean.parseBoolean(initArgs.get(HORREUM_DEV_KEYCLOAK_ENABLED))) {
            ...
            envVariables.put(
                    ...
                    keycloakEnv.get("keycloak.host")
                            .concat("/realms/")
                            .concat(HORREUM_REALM)
            );
            ...
            keycloak = KeycloakBuilder.builder()
                    ...
                    .clientId("admin-cli")
                    .resteasyClient(
                            ((ResteasyClientBuilder) ClientBuilder.newBuilder()).disableTrustManager()
                                    .build()
                    )
                    .build();
            ...
            try {
                ...
                String httpPort = config.getOptionalValue("quarkus.http.port", String.class)
                        .orElse("8080");
                String httpHost = config.getOptionalValue("quarkus.http.host", String.class)
                        .orElse("localhost");

                ClientRepresentation uiClient = keycloak.realm(HORREUM_REALM)
                        .clients()
                        .findByClientId("horreum-ui")
                        .get(0);
                uiClient.getWebOrigins()
                        .add(
                                "http://".concat(httpHost)
                                        .concat(":")
                                        .concat(httpPort)
                        );
                uiClient.getRedirectUris()
                        .add(
                                "http://".concat(httpHost)
                                        .concat(":")
                                        .concat(httpPort)
                ...
                keycloak.realm(HORREUM_REALM)
                        .clients()
                        .get(uiClient.getId())
                        .update(uiClient);

                ClientRepresentation mainClient = keycloak.realm(HORREUM_REALM)
                        .clients()
                        .findByClientId("horreum")
                        .get(0);
                envVariables.put(
                        "quarkus.oidc.credentials.secret",
                        keycloak.realm(HORREUM_REALM)
                                .clients()
                                .get(mainClient.getId())
                                .getSecret()
                                .getValue()
                );
            } catch (Exception e) {
        ...
        if (container != null) {
            ...
            try {
                waitingConsumer.waitUntil(
                        outputFrame -> outputFrame.getUtf8String()
                                .contains(pattern),
                ...
                throw new RuntimeException(
                        "Timed out waiting for " + container.getContainerName() + " container to start"
                );
            }
        } else {
        ...
        if (initArgs.containsKey(HORREUM_DEV_POSTGRES_SSL_CERTIFICATE)) {
            try {
                ...
                try (
                    ...
                    outputStream.write(
                            initArgs.get(HORREUM_DEV_POSTGRES_SSL_CERTIFICATE)
    ...
    private static String createOidcPKCS12Store(Map<String, String> initArgs) {
        try {
            ...
            X509CertificateHolder cert = (X509CertificateHolder) new PEMParser(
                    new StringReader(initArgs.get(HORREUM_DEV_KEYCLOAK_HTTPS_CERTIFICATE))
            ).readObject();
            PEMKeyPair keyPair = (PEMKeyPair) new PEMParser(
                    new StringReader(initArgs.get(HORREUM_DEV_KEYCLOAK_HTTPS_CERTIFICATE_KEY))
            ).readObject();

            KeyStore pkcsStore = KeyStore.getInstance("PKCS12", new BouncyCastleProvider());
            pkcsStore.load(null);
            pkcsStore.setKeyEntry(
                    "",
            ...
            try (
                    OutputStream outputStream = new FileOutputStream(keycloakTrustStore)
            ) {
