...
public class PostgresResource implements ResourceLifecycleManager {
    ...
    public void init(Map<String, String> initArgs) {
        boolean startDevService = !initArgs.containsKey(HORREUM_DEV_POSTGRES_ENABLED) || (initArgs.containsKey(
        ...
        if (startDevService) {
            ...
            if (initArgs.containsKey(HORREUM_DEV_POSTGRES_BACKUP)) {
                ...
                postgresContainer.addFileSystemBind(
                        initArgs.get(HORREUM_DEV_POSTGRES_BACKUP),
            ...
            props.forEach(
                    (key, val) -> postgresContainer.withParameter(
                            key.toString()
                                    .concat("=")
                                    .concat(val.toString())
                    )
            ...
            if (initArgs.containsKey(HORREUM_DEV_POSTGRES_SSL_CERTIFICATE) && initArgs.containsKey(
                ...
                postgresContainer.withCopyToContainer(
                        postgresTransferable(initArgs.get(HORREUM_DEV_POSTGRES_SSL_CERTIFICATE)),
                        certFile
                );
                postgresContainer.withCopyToContainer(
                        postgresTransferable(initArgs.get(HORREUM_DEV_POSTGRES_SSL_CERTIFICATE_KEY), 0_600),
                        keyFile
                );
            }
        }
    ...
    public Map<String, String> start(Optional<Network> network) {
        ...
        if (!prodBackup) {
            try (
                    Connection conn = DriverManager.getConnection(postgresContainer.getJdbcUrl(), "dbadmin", "secret")
            ) {
                conn.createStatement()
                        .executeUpdate("CREATE ROLE appuser noinherit login password 'secret';");
                conn.createStatement()
                        .executeUpdate("CREATE ROLE keycloak noinherit login password 'secret';");
                conn.createStatement()
                        .executeUpdate("CREATE DATABASE keycloak WITH OWNER = 'keycloak';");
                conn.createStatement()
                        .executeUpdate("GRANT ALL ON SCHEMA public TO keycloak;");
            } catch (SQLException t) {
                throw new RuntimeException(t);
            }
        }
        String postgresContainerName = postgresContainer.getContainerName()
                .replaceAll("/", "");
        Integer port = postgresContainer.getMappedPort(5432);
        String jdbcUrl = inContainer ?
                postgresContainer.getJdbcUrl()
                        .replaceAll("localhost", networkAlias)
                        .replaceAll(port.toString(), "5432") :
                postgresContainer.getJdbcUrl();
        ...
        return Map.of(
                ...
                postgresContainer.getJdbcUrl(),
                "quarkus.datasource.jdbc.url.internal",
                jdbcUrl
        );
    }
    ...
    public String getJdbcUrl() { return postgresContainer.getJdbcUrl(); }

    public PostgreSQLContainer getContainer() { return this.postgresContainer; }
