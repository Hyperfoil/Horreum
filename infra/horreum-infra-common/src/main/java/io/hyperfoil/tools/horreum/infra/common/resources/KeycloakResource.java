...
public class KeycloakResource implements ResourceLifecycleManager {
    ...
    public void init(Map<String, String> initArgs) {
        ...
        boolean startDevService = !initArgs.containsKey(HORREUM_DEV_KEYCLOAK_ENABLED) || (initArgs.containsKey(
                HORREUM_DEV_KEYCLOAK_ENABLED
        ...
        if (startDevService) {
            ...
            if (!initArgs.containsKey(HORREUM_DEV_POSTGRES_BACKUP)) {
                ...
                try (
                        InputStream is = KeycloakResource.class.getClassLoader()
                                .getResourceAsStream("keycloak-horreum.json")
                ) {
                    if (is == null) {
                ...
                ImageFromDockerfile imageFromDockerfile = new ImageFromDockerfile().withDockerfileFromBuilder(
                        builder -> builder.from(KEYCLOAK_IMAGE)
                ...
                keycloakContainer = (initArgs.containsKey(HORREUM_DEV_KEYCLOAK_CONTAINER_PORT)) ?
                        new FixedHostPortGenericContainer<>("were-going-to-override-this").withFixedExposedPort(
                                Integer.parseInt(initArgs.get(HORREUM_DEV_KEYCLOAK_CONTAINER_PORT)),
                                8080
                        ) :
                ...
                keycloakContainer = (initArgs.containsKey(HORREUM_DEV_KEYCLOAK_CONTAINER_PORT)) ?
                        new FixedHostPortGenericContainer<>(KEYCLOAK_IMAGE).withFixedExposedPort(
                                Integer.parseInt(initArgs.get(HORREUM_DEV_KEYCLOAK_CONTAINER_PORT)),
                                8080
                        ) :
                ...
                keycloakContainer.withEnv("KC_HTTP_PORT", "8080")
                        ...
                        .withEnv(
                                "KC_DB_URL",
                                "jdbc:postgresql://" + initArgs.get(HORREUM_DEV_POSTGRES_NETWORK_ALIAS) +
                                        ":5432/keycloak"
                        )
            ...
            if (initArgs.containsKey(HORREUM_DEV_KEYCLOAK_HTTPS_CERTIFICATE) && initArgs.containsKey(
                    HORREUM_DEV_KEYCLOAK_HTTPS_CERTIFICATE_KEY
            )) {
                keycloakContainer.withCopyToContainer(
                        Transferable.of(initArgs.get(HORREUM_DEV_KEYCLOAK_HTTPS_CERTIFICATE)),
                        "/tmp/keycloak-tls.crt"
                );
                keycloakContainer.addEnv("KC_HTTPS_CERTIFICATE_FILE", "/tmp/keycloak-tls.crt");
                keycloakContainer.withCopyToContainer(
                        Transferable.of(initArgs.get(HORREUM_DEV_KEYCLOAK_HTTPS_CERTIFICATE_KEY)),
    ...
    public Map<String, String> start(Optional<Network> network) {
        ...
        boolean https = keycloakContainer.getExposedPorts()
                .contains(8443);
        String mappedPort = keycloakContainer.getMappedPort(https ? 8443 : 8080)
                .toString();
        String keycloakHost = String.format(
                https ? "https://%s:%s" : "http://%s:%s",
        ...
        return Map.of(
                ...
                keycloakHost,
                "keycloak.admin.url",
                keycloakHost.concat("/realms/master/protocol/openid-connect/token"),
                "quarkus.oidc.auth-server-url",
                keycloakHost.concat("/auth/realms/horreum")
        );
    }
    ...
    public GenericContainer<?> getContainer() { return this.keycloakContainer; }
}
