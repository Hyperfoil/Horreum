...
post() {
  ...
  curl -s --fail-with-body ${API}$1 -H 'Authorization: Bearer '$TOKEN -H 'Content-Type: '$TYPE -d "$BODY" > $OUTPUT
  if [ $? != 0 ]; then
     cat $OUTPUT >&2
     echo "" >&2
     fail
...
get() {
  curl -s --fail-with-body ${API}$1 -H 'Authorization: Bearer '$TOKEN -H 'Accept: application/json' || fail
}

delete() {
  echo "DELETE $1"
  curl -s --fail-with-body ${API}$1 -H 'Authorization: Bearer '$TOKEN -X DELETE || fail
}

combine() {
  local FILE=$(mktemp --suffix .json)
  if [[ "$1" == /* ]]; then
    BASE=$1
  else
    BASE=$EX/$1
