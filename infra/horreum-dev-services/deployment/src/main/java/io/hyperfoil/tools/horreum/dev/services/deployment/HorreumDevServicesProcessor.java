...
@BuildSteps(
        onlyIfNot = IsNormal.class,
        onlyIf = {
                HorreumDevServicesProcessor.IsEnabled.class, GlobalDevServicesConfig.Enabled.class
        }
)
public class HorreumDevServicesProcessor {
    ...
    public void startHorreumContainers(
        ...
        LOG.infof(
                "Horreum dev services (enabled: ".concat(Boolean.toString(horreumBuildTimeConfig.enabled))
                        .concat(")")
        );
            ...
            try {
                ...
                if (!errors) {
                    ...
                    if (horreumKeycloakDevService == null || horreumPostgresDevService == null) {
                        ...
                        containerArgs.put(
                                HORREUM_DEV_KEYCLOAK_NETWORK_ALIAS,
                                horreumBuildTimeConfig.keycloak.networkAlias
                        );
                        containerArgs.put(
                                HORREUM_DEV_POSTGRES_ENABLED,
                        ...
                        containerArgs.put(
                                HORREUM_DEV_POSTGRES_NETWORK_ALIAS,
                                horreumBuildTimeConfig.postgres.networkAlias
                        );
                        ...
                        containerArgs.put(
                                HORREUM_DEV_KEYCLOAK_ADMIN_USERNAME,
                                horreumBuildTimeConfig.keycloak.adminUsername
                        );
                        containerArgs.put(
                                HORREUM_DEV_KEYCLOAK_ADMIN_PASSWORD,
                        ...
                        horreumBuildTimeConfig.postgres.databaseBackup.map(File::getAbsolutePath)
                                .ifPresent(
                                        backupFilename -> containerArgs.put(HORREUM_DEV_POSTGRES_BACKUP, backupFilename)
                                );
                        ...
                        if (horreumBuildTimeConfig.keycloak.httpsEnabled) {
                            SelfSignedCert keycloakSelfSignedCert = new SelfSignedCert(
                                    "RSA",
                                    "SHA256withRSA",
                                    "localhost",
                                    123
                            ...
                            containerArgs.put(
                                    HORREUM_DEV_KEYCLOAK_HTTPS_CERTIFICATE,
                                    keycloakSelfSignedCert.getCertString()
                            );
                            containerArgs.put(
                        ...
                        if (horreumBuildTimeConfig.postgres.sslEnabled) {
                            SelfSignedCert postgresSelfSignedCert = new SelfSignedCert(
                                    "RSA",
                                    "SHA256withRSA",
                                    "localhost",
                                    123
                            ...
                            containerArgs.put(
                                    HORREUM_DEV_POSTGRES_SSL_CERTIFICATE,
                                    postgresSelfSignedCert.getCertString()
                            );
                            containerArgs.put(
                                    HORREUM_DEV_POSTGRES_SSL_CERTIFICATE_KEY,
                                    postgresSelfSignedCert.getKeyString()
                            );
                        }

                        Map<String, String> envvars = HorreumResources.startContainers(
                                Collections.unmodifiableMap(containerArgs)
                        );
                        ...
                        if (horreumBuildTimeConfig.postgres.enabled) {
                            ...
                            if (horreumBuildTimeConfig.postgres.sslEnabled) {
                                ...
                                postgresConfig.put(
                                        "quarkus.datasource.jdbc.additional-jdbc-properties.sslrootcert",
                                        envvars.get("quarkus.datasource.jdbc.sslrootcert")
                                );
                            }

                            horreumPostgresDevService = new DevServicesResultBuildItem.RunningDevService(
                                    HorreumResources.postgreSQLResource.getContainer()
                                            .getContainerName(),
                                    HorreumResources.postgreSQLResource.getContainer()
                                            .getContainerId(),
                        ...
                        if (horreumBuildTimeConfig.keycloak.enabled) {
                            ...
                            horreumKeycloakDevService = new DevServicesResultBuildItem.RunningDevService(
                                    HorreumResources.keycloakResource.getContainer()
                                            .getContainerName(),
                                    HorreumResources.keycloakResource.getContainer()
                                            .getContainerId(),
    ...
    public static class IsEnabled implements BooleanSupplier {
        ...
        public boolean getAsBoolean() { return config.enabled; }
    }
}
