...
echo "QUARKUS_OIDC_AUTH_SERVER_URL=http://$KEYCLOAK_HOST:$KEYCLOAK_PORT/realms/horreum" >> /cwd/horreum-backend/.env
...
echo "HORREUM_KEYCLOAK_URL=$KEYCLOAK_BASE" >> /cwd/horreum-backend/.env

while ! curl -s --fail $KEYCLOAK_BASE; do
  echo 'Waiting for Keycloak to start.'
  sleep 5;
done
KEYCLOAK_ADMIN_TOKEN=$(curl -s $KEYCLOAK_BASE/realms/master/protocol/openid-connect/token -X POST -H 'content-type: application/x-www-form-urlencoded' -d 'username=admin&password=secret&grant_type=password&client_id=admin-cli' | jq -r .access_token)
[ -n "$KEYCLOAK_ADMIN_TOKEN" -a "$KEYCLOAK_ADMIN_TOKEN" != "null" ] || exit 1
AUTH='Authorization: Bearer '$KEYCLOAK_ADMIN_TOKEN
REALM_URL=$KEYCLOAK_BASE/admin/realms/horreum
