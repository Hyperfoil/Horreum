...
echo "QUARKUS_OIDC_AUTH_SERVER_URL=http://$KEYCLOAK_UI_HOST:$KEYCLOAK_PORT/realms/horreum" >> /cwd/horreum-backend/.env
KEYCLOAK_INTERNAL_BASE="http://$KEYCLOAK_INTERNAL_HOST:$KEYCLOAK_PORT"
KEYCLOAK_UI_BASE="http://$KEYCLOAK_UI_HOST:$KEYCLOAK_PORT"
# Keycloak URL must match QUARKUS_OIDC_AUTH_SERVER_URL or we have to set QUARKUS_OIDC_TOKEN_ISSUER
echo "HORREUM_KEYCLOAK_URL=$KEYCLOAK_UI_BASE" >> /cwd/horreum-backend/.env

while ! curl -s --fail $KEYCLOAK_INTERNAL_BASE; do
  echo 'Waiting for Keycloak to start.'
  sleep 5;
done
KEYCLOAK_ADMIN_TOKEN=$(curl -s $KEYCLOAK_INTERNAL_BASE/realms/master/protocol/openid-connect/token -X POST -H 'content-type: application/x-www-form-urlencoded' -d 'username=admin&password=secret&grant_type=password&client_id=admin-cli' | jq -r .access_token)
