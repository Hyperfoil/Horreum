...
public interface AlertingService {
    @GET
    @Path("variables")
    List<Variable> variables(@QueryParam("test")
    Integer testId);
    ...
    @DELETE
    @Path("change/{id}")
    void deleteChange(@PathParam("id")
    int id);

    @POST
    @Path("recalculate")
    void recalculateDatapoints(
            @Parameter(required = true)
            @QueryParam("test")
            int testId,
            @QueryParam("notify")
    ...
    @GET
    @Path("recalculate")
    DatapointRecalculationStatus getRecalculationStatus(
            @Parameter(required = true)
            @QueryParam("test")
            int testId
    );
    ...
    @POST
    @Path("/datapoint/last")
    List<DatapointLastTimestamp> findLastDatapoints(@RequestBody(required = true)
    LastDatapointsParams params);
    ...
    void updateChangeDetection(
            @QueryParam("testId")
            @Parameter(required = true)
            int testId,
            @RequestBody(required = true)
            AlertingService.ChangeDetectionUpdate update
    );
    ...
    List<MissingDataRule> missingDataRules(
            @Parameter(required = true)
            @QueryParam("testId")
            int testId
    );
    ...
    void deleteMissingDataRule(@PathParam("id")
    int id);
