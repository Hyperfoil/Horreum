...
public class ExperimentProfile {
    ...
    @Schema(type = SchemaType.ARRAY, implementation = String.class, required = true, description = "Array of selector labels", example = "[\"Framework\"]")
    public JsonNode selectorLabels;
    @Schema(description = "Selector filter to apply to Selector label values", example = "value => value === 'quarkus-resteasy-reactive-hibernate-reactive'")
    public String selectorFilter;
    @Schema(type = SchemaType.ARRAY, implementation = String.class, required = true, description = "Array of selector labels for comparison Baseline", example = "[\"timestamp\"]")
