...
public interface ActionService {
    ...
    @POST
    @Path("update")
    Action update(@RequestBody(required = true)
    Action action);

    @GET
    @Path("list")
    List<Action> list(@QueryParam("limit")
    Integer limit,
            @QueryParam("page")
            Integer page,
            @QueryParam("sort")
    ...
    @GET
    @Path("test/{id}")
    List<Action> getTestActions(@PathParam("id")
    int testId);
    ...
    @POST
    @Path("allowedSites")
    AllowedSite addSite(@RequestBody(required = true)
    String prefix);

    @DELETE
    @Path("allowedSites/{id}")
    void deleteSite(@PathParam("id")
    long id);
}
