...
@Extension(
        name = "x-smallrye-profile-external",
        value = ""
)
public interface TestService {
    ...
    Test get(@PathParam("id")
    int id, @QueryParam("token")
    String token);
    ...
    Test getByNameOrId(@PathParam("name")
    String input);

    @POST
    @Operation(description = "Create a new test")
    Test add(@RequestBody(required = true)
    Test test);
    ...
    TestQueryResult list(@QueryParam("roles")
    String roles,
            @QueryParam("limit")
            Integer limit,
            @QueryParam("page")
    ...
    TestListing summary(@QueryParam("roles")
    String roles, @QueryParam("folder")
    String folder,
            @DefaultValue(DEFAULT_LIMIT)
            @QueryParam("limit")
            Integer limit,
    ...
    List<String> folders(@QueryParam("roles")
    String roles);

    @POST
    @Path("{id}/addToken")
    @Operation(
            description = "Add a Test API Token for access to provide access to a test data for integrated tooling, e.g. reporting services"
    )
    @Parameters(value = {
            @Parameter(
                    name = "id",
    ...
    int addToken(@PathParam("id")
    int testId, TestToken token);
    ...
    Collection<TestToken> tokens(@PathParam("id")
    int testId);
    ...
    void dropToken(@PathParam("id")
    int testId, @PathParam("tokenId")
    int tokenId);
    ...
    void updateAccess(@PathParam("id")
    int id,
            @QueryParam("owner")
            String owner,
            @QueryParam("access")
    ...
    void updateNotifications(@PathParam("id")
    int id,
            @QueryParam("enabled")
            boolean enabled
    );
    ...
    void updateFolder(@PathParam("id")
    int id, @QueryParam("folder")
    String folder);
            ...
            @APIResponse(
                    ...
                    content = {
                            @Content(
                                    schema = @Schema(
                                            type = SchemaType.ARRAY,
                                            implementation = Fingerprints.class
                                    )
                            )
                    }
            )
    })
    List<Fingerprints> listFingerprints(@PathParam("id")
    int testId);
                    ...
                    @APIResponse(
                            ...
                            content = {
                                    @Content(
                                            schema = @Schema(
                                                    type = SchemaType.ARRAY,
                                                    implementation = ExportedLabelValues.class
                                            )
                                    )
                            }
                    )
    ...
    void updateTransformers(@PathParam("id")
    int testId, @RequestBody(required = true)
    List<Integer> transformerIds);
    ...
    void recalculateDatasets(@PathParam("id")
    int testId);
    ...
    RecalculationStatus getRecalculationStatus(@PathParam("id")
    int testId);
