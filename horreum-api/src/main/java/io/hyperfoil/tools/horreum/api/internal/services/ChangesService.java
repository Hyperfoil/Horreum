...
public interface ChangesService {
    ...
    @POST
    @Path("/search")
    String[] search(@RequestBody(required = true)
    Target query);

    @POST
    @Path("/query")
    List<TimeseriesTarget> query(@RequestBody(required = true)
    Query query);

    @Operation(hidden = true)
    @OPTIONS
    @Path("/annotations")
    Response annotations(@HeaderParam("Origin")
    String origin);

    @POST
    @Path("/annotations")
    List<AnnotationDefinition> annotations(@RequestBody(required = true)
    AnnotationsQuery query);
