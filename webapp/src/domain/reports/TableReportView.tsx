...
function RunDataView(props: RunDataViewProps) {
    ...
    return (<Popover
        headerContent={ <>Run <NavLink to={"/run/" + props.data.runId}>{ props.data.runId }</NavLink></> }
        bodyContent={<TableComposable variant="compact"><Tbody>
            { props.config.components.map((c, i) => <Tr key={i}>
                <Th>{ c.name }</Th>
                <Td>{ props.data?.values[i] }</Td>
            </Tr>) }
        </Tbody></TableComposable>}
    >
        ...
        <LevelItem style={{ width: "50%"}}>
            ...
            >
                <Thead>
                    <Tr>
                        <Th></Th>
                        { series.map(s => <Th key={s}>{ seriesFormatter(s) }</Th>)}
                    </Tr>
                </Thead>
                <Tbody>
                    { labels.map(l => <Tr key={l}>
                        <Th>{ labelFormatter(l) }</Th>
                        { series.map(s => <Td key={s}>
                            <RunDataView
                                config={ props.config }
...
function Comment(props: CommentProps) {
    ...
    if (edit) {
        return (<div className="reportComment">
            <TextArea
                id="comment"
                style={{ width: "100%" }}
                isDisabled={ updating }
        ...
        return (<div className="reportComment">
            ...
            >{ text ? "Edit" : "Add" } comment <EditIcon /></Button>
            <div>{ text }</div>
        </div>)
    }
}
...
function update(report: TableReport, comment: ReportComment | undefined, text: string, level: number, category?: string, componentId?: number) {
    if (comment === undefined) {
        comment = {
            id: -1,
            level, category, componentId,
            comment: text
        }
...
export default function TableReportView(props: TableReportViewProps) {
    ...
    return (<div>
        ...
        Created on { formatDateTime(props.report.created) } from runs in
        test <NavLink to={ "/test/" + props.report.config.test.id }>{ props.report.config.test.name + " (" + props.report.config.test.id + ")" }</NavLink>
        <Comment
        ...
        { categories.map((cat, i1) => {
            ...
            return (<React.Fragment key={i1}>
                ...
                { config.components.map((comp, i2) => {
                    const comment2 = props.report.comments.find(c => c.level === 2 && c.category === cat && c.componentId === comp.id)
