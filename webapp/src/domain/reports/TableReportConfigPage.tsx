...
export default function TableReportConfigPage() {
    ...
    useEffect(() => {
        ...
        api.getTableConfig(id).then(config => {
                setConfig(config)
                ...
                <FormSection
                    title={ <>
                        ...
                        <Popover
                            ...
                            bodyContent={<div>
                                Filtering lets you pre-select which runs are admitted to the report.
                                It is an optional feature: if you keep the list of accessors empty
                                all runs will be admitted.
                                <List>
                                    <ListItem>Accessors select data from the run based on its <NavLink to="/schema">schema</NavLink>.</ListItem>
                                    <ListItem>Filtering function takes the result of single accessor
                                        or object keyed by accessors (in case of multiple accessors)
                                        and returns <code>true</code> if the run should be admitted.</ListItem>
                                </List>
                            </div>}
                            >
                            <HelpButton />
                ...
                <FormSection
                    title={ <>
                        ...
                        <Popover
                            ...
                            bodyContent={<div>
                                Categories split runs to several groups where side-by-side comparison doesn't make sense.
                                For each category and each component (e.g. 'Throughput' or 'CPU Usage') there will be one
                                table/chart of results.
                                If this is left empty all runs will be grouped into single unnamed category.
                                <List>
                                    <ListItem>Accessors select data from the run based on its <NavLink to="/schema">schema</NavLink>.</ListItem>
                                    <ListItem>Category function takes the result of single accessor
                                        or object keyed by accessors (in case of multiple accessors)
                                        and produces a value that would be used for the distinction.</ListItem>
                                    <ListItem>Category formatter takes the result of category function
                                        (or the accessors if the function is not present) and formats it
                                        for presentation.</ListItem>
                                </List>
                            </div>}
                            >
                            <HelpButton />
                    ...
                    title={ <>
                        ...
                        <Popover
                            ...
                            bodyContent={<div>
                                ...
                                presented as the columns in table or lines in the chart.
                                Selector for series is mandatory.
                                <List>
                                    <ListItem>Accessors select data from the run based on its <NavLink to="/schema">schema</NavLink>.</ListItem>
                                    <ListItem>Series function takes the result of single accessor
                                        or object keyed by accessors (in case of multiple accessors)
                                        and produces a value that would be used to select the series.</ListItem>
                                    <ListItem>Series formatter takes the result of series function
                                        (or the accessors if the function is not present) and formats it
                                        for presentation.</ListItem>
                                </List>
                            </div>}
                    ...
                    title={ <>
                        ...
                        <Popover
                            ...
                            bodyContent={<div>
                                ...
                                <List>
                                    <ListItem>Accessors select data from the run based on its <NavLink to="/schema">schema</NavLink>.</ListItem>
                                    <ListItem>Label function takes the result of single accessor
                                        or object keyed by accessors (in case of multiple accessors)
                                        and produces a value that would be used for the label.</ListItem>
                                    <ListItem>Label formatter takes the result of label function
                                        (or the accessors if the function is not present) and formats it
                                        for presentation.</ListItem>
                                </List>
                            </div>}
                    ...
                    <Button
                        ...
                        onClick={ () => {
                            api.previewTableReport(config).then(
                                ...
                                error => dispatch(alertAction('PREVIEW_REPORT', "Failed to generate report preview.", error))
                            )
                        }}
                    >Preview</Button>
                </ActionGroup>
