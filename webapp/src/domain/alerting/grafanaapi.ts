...
export const fetchDatapoints = (variableIds: number[], tags: string, from: number, to: number): Promise<TimeseriesTarget[]> => {
    const query = {
        range: range(from, to),
...
export const fetchAllAnnotations = (variableIds: number[], tags: string, from: number, to: number): Promise<Annotation[]> => {
