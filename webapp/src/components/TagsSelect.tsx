...
export default function TagsSelect(props: TagsSelectProps) {
    ...
    useEffect(() => {
        ...
        fetchTags(testId, props.includeTrashed || false).then(
            (response: any[]) => {
    ...
    filtered.map(t => ({ ...t, toString: () => convertTags(t) })).forEach(o => options.push(o))
    ...
    return (
        ...
        >
            {options.map((tags: SelectOptionObject | string, i: number) => (
                <SelectOption key={i} value={tags} />
            ))}
        </Select>
    )
}
