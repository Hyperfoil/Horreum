...
export default function RecalculateModal({ title, recalculate, cancel, message, isOpen, onClose, testId, showLog } : RecalculateModalProps) {
    ...
    const [recalcDatapoints, setRecalcDatapoints] = useState(true)
                ...
                {progress < 0 && (
                    <Form isHorizontal>
                        ...
                        <FormGroup label="Recalculate Datpoints:" fieldId="recalcDatapoints">
                            <Checkbox id="recalcDatapoints" isChecked={recalcDatapoints} onChange={(_event, val) => setRecalcDatapoints(val)} />
                        </FormGroup>
